...
![版本](https://img.shields.io/badge/版本-0.13.1-3eb477)
...
- **所见即所得编辑**（0.12.0 起）：输入框与卡片编辑框都是 TipTap 富文本——打 `**加粗**`、`行内代码`、`- [ ] `、` ``` ` 当场变成成品，复选框可直接点击勾选，图片节点内嵌显示；存储仍是纯 Markdown（mdToDoc / docToMd 纯函数转换，「打开→不动→保存」无损），`#标签` 自动补全照旧，输入的标签即时主题色显示（0.13.2 起）
- **格式工具栏**（0.13.0 起）：编辑区下方一排图标按钮——标签（#）、加粗、行内代码、无序 / 有序列表、任务清单、插入图片（走系统文件选择框，与粘贴上传同一条链路）；按钮不抢编辑器焦点，命令作用于当前光标 / 选区，格式按钮在选区内生效时呈高亮激活态
- **一键复制**（0.13.1 起）：卡片与回顾弹窗各有「复制」按钮，把笔记原文（纯 Markdown）写入剪贴板，点击后短暂显示「已复制 / 复制失败」反馈；写入走 Tauri 剪贴板插件（Rust 侧），不依赖 WebView 剪贴板权限
